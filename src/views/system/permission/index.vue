<template>
    <PageWrapper>
        ...
        <MoneyRR :money-crud="moneyCrud">
            <el-input v-model.number="moneyCrud.query.condition" placeholder="名称/编码" class="md:!w-52"
                      @keyup.enter.native="moneyCrud.doQuery" />
            <el-select v-model="moneyCrud.query.permissionType" clearable placeholder="资源类型" class="md:!w-52">
        ...
        <MoneyForm :money-crud="moneyCrud" :inline="true" :rules="rulesMap[type]" :dialog-class="'!w-11/12 md:!w-5/12'">
            ...
            <el-form-item v-if="type !== 'BUTTON'" label="图标" prop="icon" class="w-full">
                <icon-select class="!w-full" @selected="icon => moneyCrud.form.icon = icon" :default-icon="moneyCrud.form.icon" />
            </el-form-item>
...
import permissionApi from "@/api/module/permission.js"
...
onBeforeMount(async () => {
    ...
    permissionsTree.value = [{id: 0, permissionName: '顶级类目', children: permissions.value}]
})
...
moneyCrud.value.Hook.beforeToEdit = () => {
    type.value = moneyCrud.value.form.permissionType
}
</script>