<template>
    <el-button plain type="warning" size="small" v-if="moneyCrud.optShow.edit"
               :disabled="scope.$index === -1 || moneyCrud.rowOptDisabled.edit(scope.row)" @click="moneyCrud.toEdit(scope.row)">
        <el-icon>
            <Edit />
        </el-icon>
    </el-button>
    <el-popconfirm :title="delConfirmMsg" confirm-button-text="确定" cancel-button-text="取消" width="200"
                   @confirm="moneyCrud.doDel([scope.row])">
        <template #reference>
