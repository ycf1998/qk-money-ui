...
class MoneyCrud {
    ...
    constructor(option) {
        ...
        if(this.queryOnCreated) this.doQuery()
    }
    ...
    doQuery = (notResetPage) => {
        if (this.crudMethod.list) {
            ...
            this.crudMethod.list(query).then(res => {
                if (import.meta.env.VITE_ONLY_UI) return
                if (this.isPage) {
